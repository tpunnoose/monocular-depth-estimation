...
class StereoDepthLoss(object):
    def __init__(self, K, b, height, width, batch_size, alpha=0.85):
    ...
    def calculate_loss(self, inputs, outputs):
        ...
        loss += self.alpha*kornia.losses.ssim_loss(image_l, image_l_predicted, 3)
        loss += (1 - self.alpha)*torch.mean(torch.abs(image_l - image_l_predicted))
        
        # Right Photometric Consistency
        loss += kornia.losses.ssim_loss(image_r, image_r_predicted, 3)
        loss += (1 - self.alpha)*torch.mean(torch.abs(image_r - image_r_predicted))
        
        # Inverse Depth Smoothness Loss
        loss += kornia.losses.inverse_depth_smoothness_loss(disparity_l, image_l.float())
        loss += kornia.losses.inverse_depth_smoothness_loss(disparity_r, image_r.float())
